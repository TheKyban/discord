...
export const ServerChannel: FC<serverChannelProps> = ({
    channel,
    server,
    role,
}) => {
    ...
    return (
        ...
        >
            <Icon
                className={
                    "flex-shrink-0 w-5 h-5 text-zinc-500 dark:hover:bg-zinc-400"
                }
            />
            <p
                className={cn(
                    "line-clamp-1 font-semibold text-sm text-zinc-500 group-hover:text-zinc-300 dark:text-zinc-400 dark:group-hover:text-zinc-300 transition",
                    params?.channelId === channel.id &&
